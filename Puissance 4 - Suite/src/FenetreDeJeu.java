	...
	public void actionPerformed (ActionEvent arg0) {
		...
		if (arg0.getSource() == B[k]) {
			...
			if (h==taillec) { fen.setText( "Colonne pleine, " + tab[positionactive].nom + " recommencez !");
					...
					else {if (positionactive==n-1) {positionactive=0;}
				       else                     {positionactive++;}
					   fen.setText(tab[positionactive].nom + " à vous de jouer :");}
					...
					while (ta==true) {
						if (tab[positionactive].type==1) {ta=false;}
						else { int s1 = jeu.meilleurJeu(positionactive,tab);
						       int s2 = jeu.grille[s1].getRemplissage();
							
						       etat = jeu.jouer(s1,tab[positionactive]);
						
									grilleGraph[s2][s1].changerCouleurCase(tab[positionactive].couleur);
									grilleGraph[s2][s1].repaint();
